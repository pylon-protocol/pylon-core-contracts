...
pub fn instantiate(
    ...
    let config = Config {
        pylon_token: CanonicalAddr::from(vec![]),
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::RegisterContracts { pylon_token } => register_contracts(deps, pylon_token),
...
pub fn register_contracts(deps: DepsMut, pylon_token: String) -> Result<Response, ContractError> {
    let mut config: Config = config_read(deps.storage).load()?;
    if config.pylon_token != CanonicalAddr::from(vec![]) {
        return Err(ContractError::Unauthorized {});
    }

    config.pylon_token = deps.api.addr_canonicalize(&pylon_token)?;
    config_store(deps.storage).save(&config)?;
...
pub fn query(deps: Deps, _env: Env, msg: QueryMsg) -> Result<Binary, ContractError> {
    match msg {
        ...
        QueryMsg::Stakers {
            ...
            limit,
            order,
        } => Ok(to_binary(&query_stakers(deps, start_after, limit, order)?)?),
